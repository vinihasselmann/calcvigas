...
def export_memorial_pdf(df: pd.DataFrame) -> bytes:
    """Gera um PDF simples com os principais resultados das vigas."""
    pdf = _Pdf()
    beams = _beam_rows(df)
    if beams.empty:
        page = pdf.page()
        page.text(MARGIN, PAGE_H - MARGIN, "Memorial de calculo", 16, bold=True)
        page.text(MARGIN, PAGE_H - MARGIN - 24, "Nao ha vigas nos resultados filtrados.", 10)
        return pdf.render()
...
def _beam_rows(df: pd.DataFrame) -> pd.DataFrame:
    if "tipo_elemento" not in df:
        return pd.DataFrame()
    return df[df["tipo_elemento"].isin(["VPL", "VPT", "VR"])].copy()
